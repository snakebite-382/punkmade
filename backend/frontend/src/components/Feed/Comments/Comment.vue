<template>
    <div class="comment tw-my-2">
        <span
        class="author tw-text-base">{{ comment.author }}</span>:
        
        <span class="content tw-text-sm">{{ comment.content }}</span>

        <span class="reply-interaction tw-text-sm">
            <StyledBtn 
                @click="handleReply" 
                class="tw-p-0.5 tw-mx-2">{{ (isReplying()) ? 'stop replying' : 'reply'}}
            </StyledBtn>,
            <StyledBtn 
                @click="showReplies = !showReplies" 
                class="tw-p-0.5 tw-mr-2"> {{ showReplies ? 'stop viewing replies' : 'view replies' }}
            </StyledBtn>
        </span>
        <span class="like-interaction">
            {{ comment.likes }}
            <LikeButton 
                @post-liked="feedStore.likeComment(expectedParents)" 
                :size='18' :fill="comment.liked ? 'red' : 'none'" outline="black"
            />
        </span>
